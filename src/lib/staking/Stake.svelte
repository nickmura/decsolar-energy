          ...
          <div class="bg-white rounded-xl p-4 w-80 bg-slate-50 shadow-xl">
            <div class='w-full'>
              <div class='flex justify-between'>
                ...
                <div class='text-md font-bold mb-4 text-gray-500 italic'>Current APR: 15.9%</div>
              </div>
              ...
              <div class='flex flex-row mr-2'>Amount: <input type='number' bind:value={P2PStaked} min='0' max={Number(formatEther(P2PBalance))} class='w-fit flex 
                    font-bold w-16 max-w-32 outline-none active:appearance-none [appearance:textfield] no-spinner ml-2 mr-2' />{P2PStaked ? 'P2P' : ''}</div>
              <div class='text-[10px] text-gray-800 italic'>Balance Staked: { ((Number(P2PStaked) / Number(formatEther(P2PBalance))) * 100).toFixed(1)}%</div>
              <div class='mt-2  flex justify-center mt-4'>Staking Period</div>
                <div class='flex flex-row justify-center gap-6'>
                    <div class='flex flex-col mt-4' ><input type='radio' name='staking' class='' value=21 checked><label class='' for='21'>21d</label></div>

                    <div class='flex flex-col mt-4 opacity-50'><input type='radio' name='staking' class='' value=90><label class='' for='90' >90d</label></div>
                    <div class='flex flex-col mt-4 opacity-50'><input type='radio' name='staking' class='' value=180><label class='' for='180'>180d</label></div>
                    <div class='flex flex-col mt-4 opacity-50'><input type='radio' name='staking' class='' value=365><label class='' for='365'>365d</label></div>
                </div>
                {#if P2PStaked}
                  <div class='flex flex-col mt-4' transition:slide>
                    <hr class='w-full h-[3px] bg-grey-900 mt-4'/>
                    <div class='flex flex-wrap'>
                      Total Stake: <div class='font-bold ml-2'>{(Number(P2PStaked)).toLocaleString()} P2P</div>
                    </div>
                    <div class='flex flex-wrap'>
                      Current APY/APR: <div class='font-bold ml-2'>15.9%</div>
                    </div>
                    <div class='flex flex-wrap'>
                      ROI After 1 year: <div class='font-bold ml-2'>{((Number(P2PStaked) * 1.159)).toLocaleString()} P2P</div>
                    </div>
                    <div class='flex flex-wrap'>
